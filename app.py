...
with tab_matutino:
    ...
    # --------------------------------------------------------------------------
        ...
        if uploaded_matutino is not None:
            try:
                df_core = pd.read_excel(uploaded_matutino)
                df_core.columns = [str(c).strip() for c in df_core.columns]

                # Obtener columna P1 (índice 15)
                col_crono = df_core.columns[15] if len(df_core.columns) >= 16 else "CRONOLOGIA DEL EVENTO"

                # Filtrar subcategorías buscando texto contenido
                serie_crono = df_core[col_crono].dropna().astype(str)
