...
# ==============================================================================
# MÓDULO 1: REPORTE DE EVENTOS PENDIENTES (CÓDIGO ANTERIOR INTACTO)
# ==============================================================================
with tab_pendientes:
    ...
    # --------------------------------------------------------------------------
        ...
        if uploaded_matutino is not None:
            try:
                ...
                if "FECHA INICIO" in df_core_raw.columns:
                    df_core_raw["Fecha_DT"] = pd.to_datetime(df_core_raw["FECHA INICIO"], dayfirst=True, errors="coerce")
                else:
                    df_core_raw["Fecha_DT"] = pd.to_datetime(df_core_raw.iloc[:, 0], dayfirst=True, errors="coerce")
                
                df_core = df_core_raw.dropna(subset=["Fecha_DT"]).copy()

                # Selector de fecha para RED CORE
                default_date_core = df_core["Fecha_DT"].max().date() if not df_core.empty else datetime.date.today()
                selected_date_core = st.date_input(
                ...
                df_core_filtered = df_core[df_core["Fecha_DT"].dt.date == selected_date_core]
                ...
                # Obtener columna P1 (índice 15)
                col_crono = df_core_filtered.columns[15] if len(df_core_filtered.columns) >= 16 else "CRONOLOGIA DEL EVENTO"

                # Filtrar subcategorías buscando texto contenido en la fecha seleccionada
                ...
                icx_list = serie_crono[serie_crono.str.contains("INTERCONEXION", case=False, na=False)].tolist()

                txt_dwdm = "\n".join([f"-{val}" for val in dwdm_list]) if dwdm_list else "-NINGUNO"
                txt_isp = "\n".join([f"-{val}" for val in isp_list]) if isp_list else "-Ninguno"
                txt_icx = "\n".join([f"-{val}" for val in icx_list]) if icx_list else "- Ninguno"
